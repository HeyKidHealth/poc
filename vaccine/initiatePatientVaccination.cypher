...
(dose:VaccineDose {vaccineName:'Hepatite B'})
...
ON CREATE SET my.uuid = apoc.create.uuid(),
  my.fromExpectedDate = apoc.date.convertFormat(toString(date({month: date(dob.date).month, year: date(dob.date).year, day: date(dob.date).day}) + duration({months: dose.toAgeInMonths})), 'date', 'dd/MM/YYYY'),
  my.toExpectedDate = apoc.date.convertFormat(toString(date({month: date(dob.date).month, year: date(dob.date).year, day: date(dob.date).day}) + duration({months: dose.toAgeInMonths + 1}) + duration({days: 15})), 'date', 'dd/MM/YYYY')
